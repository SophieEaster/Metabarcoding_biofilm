...
# Functional stability despite structural changes in freshwater biofilm communities exposed to an antibiotic and an herbicide – the role of nutrient levels and season 

Sophie Oster, Eric Bollinger, Tobias Schmitt, Verena C. Schreiner, Sabine Filker, Mirco Bundschuh
...
Four chronic (14 days) exposure experiments were conducted in an indoor stream microcosm facility with lotic biofilm in the RPTU in Landau, Germany. These studies were conducted with four chemical treatments and two different nutrient media:
...
As functional endpoints the two biomass parameters ash free dry weight (AFDW) and the photosynthetic pigments (chlorophyll a, b & c) were measured. As structural endpoints DNA metabarcoding was conducted to investigate the prokaryotic and eukaryotic communities. 
...
prok_analyzed %>%
  ...
  scale_shape_manual(
    values = c("Control" = 21, "Ciprofloxacin" = 22, "Propyzamide" = 25)  
  ) +
...
prok_barplot_proteobacteria <- prokaryotes_long %>%
  ...
  ggplot(aes(x = sample, y = RA, fill = family)) +
  geom_bar(stat = "identity") +
...
euk_barplot_chlorophyta <- eukaryotes_long %>%
  ...
  dplyr::filter(!is.na(RA) & !is.na(family)) %>%  
  ggplot(aes(x = sample, y = RA, fill = family)) +
  geom_bar(stat = "identity") +
  labs(x = "Sample", y = "Abundance", fill = "Family") +
  theme_minimal() +
