...
# statistics
kruskal_diversity_indices_prok <- prok_diversity %>%
  group_by(exp, name) %>%
  nest() %>%
  mutate(
    kruskal_p = map_dbl(data, ~ kruskal.test(value ~ concentration_ug_L, data = .x)$p.value)
  )

print(kruskal_diversity_indices_prok)
...
# statistics
kruskal_diversity_indices_euk <- euk_diversity %>%
  group_by(exp, name) %>%
  nest() %>%
  mutate(
    kruskal_p = map_dbl(data, ~ kruskal.test(value ~ concentration_ug_L, data = .x)$p.value)
...
print(kruskal_diversity_indices_euk)
